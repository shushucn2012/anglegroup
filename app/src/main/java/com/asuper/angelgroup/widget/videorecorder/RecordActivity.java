package com.asuper.angelgroup.widget.videorecorder;
...
import com.asuper.angelgroup.moduel.home.FhVideoPublishActivity;

import java.io.File;
import java.io.IOException;
import java.util.List;

import static com.asuper.angelgroup.widget.videorecorder.CommonUtils.SIZE_1;
import static com.asuper.angelgroup.widget.videorecorder.CommonUtils.SIZE_2;
import static com.asuper.angelgroup.widget.videorecorder.CommonUtils.getScreenHeight;
import static com.asuper.angelgroup.widget.videorecorder.CommonUtils.getScreenWidth;
...
public class RecordActivity extends Activity implements SurfaceHolder.Callback, View.OnClickListener {
    ...
    ImageView recordButton, video_close;
    View view_focus, view_default;
    ...
    private void initView() {
        ...
        swicthCamera = (ImageView) findViewById(R.id.swicth_camera);
        recordButton = (ImageView) findViewById(R.id.record_button);
        video_close = (ImageView) findViewById(R.id.video_close);
        ...
        video_close.setOnClickListener(this);
        doStartSize();
    ...
    private void endRecordUI() {
        ...
        recordButton.setImageResource(R.drawable.paisheshipin);   //录制按钮变成待停止
    }
            ...
            if (startRecord()) {
                ...
                videoTime.setOnChronometerTickListener(new Chronometer.OnChronometerTickListener() {
                    ...
                    public void onChronometerTick(Chronometer chronometer) {
                        ...
                        count++;
                        timeStr = chronometer.getText().toString();
                        view_focus.setLayoutParams(new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT, count));
                        view_default.setLayoutParams(new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT, 2 * 60 - count));
                    }
                });
            }
        } else {
    ...
    private void endRecord() {
        ...
        videoTime.stop();
        videoTime.setBase(SystemClock.elapsedRealtime());
        if (FU.paseInt(timeStr.substring(timeStr.length() - 1)) < 5 && count < 10) {
            Toast.makeText(RecordActivity.this, "视频长度不能小于5秒", Toast.LENGTH_SHORT).show();
            count = 0;
            return;
        }
        count = 0;
        Intent intent = new Intent(this, FhVideoPublishActivity.class);
        intent.putExtra("videopath", videoFile.getAbsolutePath());
        intent.putExtra("videoduration", timeStr);
