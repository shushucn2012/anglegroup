...
public class AliyunUploadUtils {
	...
	public void uploadPic(String filePath, OnUploadFinish onUploadFinish) {
		...
		String objectKey = "client/" + UUID.randomUUID().toString() + expandedName;
//		String objectKey = UUID.randomUUID().toString() + expandedName;
		...
		StringNetRequest.getInstance(mContext).startRequest(wholeUrl, Request.Method.POST, requestBodyData, 0, aliyunlsner);
	}
	...
	private void submitPut(String url, String filepath) {
		...
		try {
			...
			String ContentType = FileTypeAnalysis.getContentType(FileTypeAnalysis.getFileByFile(file));
...
//			String objectKey = UUID.randomUUID().toString() + expandedName;
		...
		String requestBodyData = ParamBuild.getAliyunUrl(piclist);
		StringNetRequest.getInstance(mContext).startRequest(wholeUrl, Request.Method.POST, requestBodyData, 0, aliyunListlsner);
	}
